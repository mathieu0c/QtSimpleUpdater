...
inline bool checkFile(const QString& inFilename, const CryptoPP::RSASSA_PKCS1v15_SHA_Verifier& verifier,
  ...
  if (signature.empty()) {
    SPDLOG_CRITICAL("Cannot read signature file:{} (abort)", signatureFile);
    return false;
  }
  SPDLOG_DEBUG("Read signature file from <{}>", signatureFile);
  SPDLOG_DEBUG("Read signature (size: {}) <{}>", signature.size(), signature);
...
inline std::optional<bool> checkFileSignature(const QString& fileToCheck, const QString& publicKeyPath,
  ...
  if (!verifKeyOpt) {
    SPDLOG_ERROR("Aborting installation as we could not load the specified verifier (public) key file: <{}>",
                 publicKeyPath);
    return {};
  }
