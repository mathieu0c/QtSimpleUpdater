...
void UpdateHandler::downloadAndInstall(bool onlyRetrieveManifest) {
  ...
  if (manifestUrl.isEmpty()) {
    resetState();
    SPDLOG_ERROR("Could not retrieve the manifest url from github");
    showInfoMessage(InfoBoxMsgType::kWarning, tr("Could not retrieve manifest url from github"));
    return;
  }

  SPDLOG_INFO("Github manifest url: <{}>", manifestUrl);
...
void UpdateHandler::onLatestUpdateRetrieved(std::optional<ReleaseInfo> releaseInfoOpt, bool installAfterwards) {
  ...
  SPDLOG_INFO("Release infos: <{}>", releaseInfo);
  SPDLOG_INFO("--- Update status ---\n\tRunning version: {}\n\tDistant version: {}",
              m_kRunningVersion,
              releaseInfo.versionAvailable);

  if (m_kRunningVersion >= releaseInfo.versionAvailable) {
    SPDLOG_INFO("No installation candidate found (distant version <= running version)");
  ...
  SPDLOG_INFO("Installation candidate found (distant version > running version)");
  ...
  if (m_showInstallPropositionOnNextOccasion) {
    ...
    if (ans == QMessageBox::Yes) {
      SPDLOG_INFO("User chose to install the new version: {}", releaseInfo.versionAvailable);
      downloadAndInstall(false);
      if (!this->isVisible()) {
        this->show();
      }
    } else {
      SPDLOG_INFO("User chose NOT to install the new version: {}", releaseInfo.versionAvailable);
    }
...
void UpdateHandler::onManifestRetrieved(std::optional<QJsonDocument> docOpt, bool install) {
  ...
  if (!docOpt) {
    resetState();
    SPDLOG_ERROR("Could not retrieve manifest JSON document");
    showInfoMessage(InfoBoxMsgType::kWarning, tr("Could not retrieve update manifest. Retry later."));
    return;
  }
  SPDLOG_INFO("Successfully retrieved manifest JSON document");
  if (!utils::json::save(docOpt.value(), m_kDistantManifestName)) {
    resetState();
    SPDLOG_ERROR("Could not save manifest file");
    return;
  }
  auto manifestOpt{getManifest(docOpt.value())};
  if (!manifestOpt) {
    resetState();
    SPDLOG_ERROR("Could not read a valid manifest file from the retrieved manifest");
    return;
  }
  ...
  if (!m_latestReleaseInfoOpt) {
    SPDLOG_ERROR("Latest release info could not be found. Unexpected situation - abort update process");
    return;
  }
...
void UpdateHandler::updateFromInstaller(const Manifest& manifest) {
  ...
  if (installerLink.isEmpty()) {
    SPDLOG_ERROR("Could not find link for installer");
  ...
  SPDLOG_INFO("Downloading installer at <{}>", installerLink);
  ...
  connect(dlManager,
          ...
          [=, this](int32_t successCount, const QVector<updt::net::FailedDownload>& fails) {
            SPDLOG_DEBUG("Success: <{}>  Fails: <{}>", successCount, fails.size());
            for (const auto& e : fails) {
              SPDLOG_WARN("\t Failed to dl <{}> because of <{}>", e.url, e.errMsg);
            }
            if (fails.size() != 0) {
              this->resetState();
            ...
            if (!safeToInstallOpt || !safeToInstallOpt.value()) {
              SPDLOG_ERROR("Failed to validate installer signature");
...
void UpdateHandler::updateFromPackage() {
  ...
  if (updatePackageLink.isEmpty()) {
    SPDLOG_ERROR("Could not find update archive link");
    showInfoMessage(InfoBoxMsgType::kWarning,
  ...
  SPDLOG_INFO("Downloading the update package at: <{}>", updatePackageLink);
...
void UpdateHandler::onUpdatePackageRetrieved() {
  ...
  if (!startProcessResult.has_value()) {
    SPDLOG_CRITICAL("Failed to start the update process");
    showInfoMessage(InfoBoxMsgType::kCritical, tr("Cannot start the update process for an unknown reason"));
    resetState();
    return;
  }
  SPDLOG_INFO("Started update with PID: <{}>", startProcessResult.value());
  QApplication::exit(0);
}
...
void UpdateHandler::onInstallerRetrieved(const QString& installer_path) {
  ...
  SPDLOG_DEBUG("Installer downloaded at: <{}>", installer_path);
