...
bool saveBA(const QByteArray& ba, const QString& path) {
  ...
  SPDLOG_DEBUG("Saving to <{}> ({} bytes)", path.toStdString(), ba.size());
...
QString Compressor::compress(const QString& outPath, QString workingDir) {
  ...
  if (!success) {
    SPDLOG_ERROR("Could not open output file: {}", outPath);
    return QString{};
  }
  QDataStream stream{&buff};
  ...
  SPDLOG_INFO("Planning on compressing to archive <{}> -> {} files", outPath, fileCount);

  for (const auto& p : m_customPaths) {
    QString pathToSend{getFilePathFromWorkingDir(p, workingDir)};

    SPDLOG_DEBUG("\t\tAdding to archive: <{}>", pathToSend);
  ...
  SPDLOG_DEBUG("Compressed BA size: <{}> bytes", ba.size());
...
bool Compressor::uncompress(const QString& path, const QString& outFolderPath) {
  ...
  if (!compressedArchive.open(QIODevice::ReadOnly)) {
    SPDLOG_ERROR("Can't open archive file: <{}>", path);
    return false;
  }
  ...
  if (!success) {
    SPDLOG_ERROR("Can't open uncompressed archive file: <{}>", path);
    return false;
  }
  QDataStream fileStream{&fileBuff};

  SPDLOG_DEBUG("Opening archive file: <{}> -> <{}> bytes", path, fileBuff.size());

  qint64 fileCount{};
  fileStream >> fileCount;

  SPDLOG_INFO("Planning on extracting from archive <{}> -> <{}> files", path, fileCount);
  ...
  for (qint64 i{}; i < fileCount; ++i) {
    ...
    if (!success) {
      SPDLOG_ERROR("Can't open compressed element in archive file: <{}>", path);
      return false;
    }
...
bool Compressor::compressToStream(const QString& path, QDataStream& stream) {
  ...
  if (!inf.exists()) {
    SPDLOG_ERROR("File does not exists: <{}>", inf.absoluteFilePath());
  } else
    SPDLOG_ERROR("Unknown input type: <{}>", inf.absoluteFilePath());
  return false;
}
...
bool Compressor::compressFolder(const QString& root, const QString& path, QDataStream& stream) {
  ...
  while (it.hasNext()) {
    it.next();
    SPDLOG_DEBUG("\t\tCompressed file <{}> to <{}>", it.filePath(), fs::getFilePathFromRoot(root, it.filePath()));
...
bool Compressor::compressFile(const QString& path, QDataStream& stream) {
  SPDLOG_DEBUG("Compressed file <{}> to <{}>", path, QFileInfo{path}.fileName());
